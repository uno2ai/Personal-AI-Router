...
On macOS, the firewall helper is **optional**. Choose **Skip** in its setup
dialog to continue without installing it or changing firewall rules. The choice
is remembered across launches. You can retry from **Settings → Optional macOS
firewall helper → Configure**. Helper setup requires an NVIDIA-signed build and
macOS administrator approval; unsigned personal builds should skip it. If your
paired devices already connect (for example, through Tailscale), Codex Pair does
not require this helper. Skipping does not disable the firewall or remove an
