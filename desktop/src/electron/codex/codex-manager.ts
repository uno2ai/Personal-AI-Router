...
export class CodexManager {
    ...
    configureWorker(input: CodexWorkerConfigInput): CodexDesktopState {
        ...
        const next: CodexConfig = {
            ...
            codexExecutable:
                ...
                (config.codexExecutable || process.env['CODEX_CLI_PATH'] || 'codex'),
    ...
    async listTasks(): Promise<CodexTaskMetadata[]> {
        ...
        if (!Array.isArray(response.tasks))
            throw new Error('Supervisor returned an invalid task list')
    ...
    private workerState(
        ...
        try {
            const descriptor = JSON.parse(readProtectedConfig(descriptorPath) ?? 'null') as Record<
                string,
                unknown
            >
            const expiresAt =
                typeof descriptor.expiresAt === 'string' ? Date.parse(descriptor.expiresAt) : 0
            if (!expiresAt || expiresAt <= Date.now()) {
                return {
                    ...
                    bootEpoch: null,
                    policyRevision: config.policyRevision,
                    error: 'Worker runtime descriptor is expired'
                }
            }
            const descriptorError =
                typeof descriptor.error === 'string' ? descriptor.error : undefined
            ...
            return {
                ...
                workerInstanceId:
                    typeof descriptor.workerInstanceId === 'string'
    ...
    private writeManagedWorkerConfig(config: CodexConfig): void {
        ...
        writeProtectedConfig(filePath, `${JSON.stringify(workerConfig, null, 2)}\n`)
    }
        ...
        for (const name of fs.readdirSync(directory)) {
            ...
            try {
                const registry = JSON.parse(
                    readProtectedConfig(path.join(directory, name)) ?? 'null'
                ) as {
    ...
    private async callManagement(
        ...
        for (const name of registries) {
            try {
                ...
                if (registry.schemaVersion !== 1 || typeof registry.socketPath !== 'string')
                    continue
