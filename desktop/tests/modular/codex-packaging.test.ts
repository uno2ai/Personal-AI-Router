...
describe('Codex packaging contract', () => {
    it('ships Worker and Supervisor exactly once with distinct lifecycle owners', () => {
        ...
        const worker = MODULAR_RUNTIME_BINARIES.find(item => item.baseName === 'nvpair-codex-worker')
        expect(worker).toMatchObject({ processName: 'codex-worker', launchOwner: 'broker', optional: true })
        expect(MODULAR_RUNTIME_BINARIES.some(item => item.baseName === 'nvpair-codex-supervisor')).toBe(false)
        expect(MODULAR_BUNDLED_BINARIES.filter(item => item.baseName === 'nvpair-codex-supervisor')).toHaveLength(1)
    })
