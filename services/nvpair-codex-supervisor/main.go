...
func main() {
	...
	targets, err := parseWorkerTargets(*workerURL, *workerEndpoints, *workerToken, *clusterDir, *discoveryFile)
	if err != nil {
		log.Fatal(err)
	}
	if err := NewMCPServerWithWorkers(targets).Serve(os.Stdin, os.Stdout); err != nil {
		log.Printf("Supervisor stopped: %v", err)
	}
}

func parseWorkerTargets(loopbackURL, endpoints, token, clusterDir, discoveryFile string) ([]WorkerTarget, error) {
	...
	if len(targets) == 0 {
		return nil, fmt.Errorf("--worker-url or --worker-endpoints is required")
	}
	return targets, nil
}
